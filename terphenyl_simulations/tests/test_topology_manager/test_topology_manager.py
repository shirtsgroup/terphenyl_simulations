...
class TestTopologyManager(unittest.TestCase):
    ...
    def test_init_new_topology_manager(
        self, mock_pickle_dump, mock_open, mock_exists, mock_isdir, mock_makedirs
    ):
        # Test initialization when the topology manager does not exist
        with patch("builtins.print") as mock_print:
            tm = TopologyManager(
                topology_dir="test_dir", topology_object="test_manager.pkl"
            )
            mock_isdir.assert_called_once_with("test_dir")
            mock_makedirs.assert_called_once_with("test_dir")
            mock_open.assert_called_once_with("test_dir/test_manager.pkl", "wb")
            mock_pickle_dump.assert_called_once()
            mock_print.assert_any_call(
                "Creating a new TopologyManager:", "test_dir/test_manager.pkl"
            )
    ...
    def test_init_load_existing_topology_manager(
        ...
        with patch("builtins.print") as mock_print:
            ...
            mock_open.assert_called_once_with("test_dir/test_manager.pkl", "rb")
            mock_pickle_load.assert_called_once()
            mock_print.assert_any_call(
                "Loading TopologyManager:", "test_dir/test_manager.pkl"
            )

    @patch("builtins.open", new_callable=mock_open)
    @patch("yaml.safe_load", return_value={"mock_key": "mock_value"})
    @patch("pickle.load", return_value={"mock_key": "mock_value"})
    @patch("json.dumps", return_value='{"mock_key": "mock_value"}')
    def test_get_build_json(
        self, mock_json_dumps, mock_picle_load, mock_yaml_load, mock_open
    ):
    ...
    def test_get_entry_dir_id(
        self, mock_open, mock_pickle_load, mock_yaml_load, mock_json_dumps, mock_uuid5
    ):
    ...
    def test_check_file_type(
        self, mock_makdirs, mock_copy, mock_get_entry_dir_id, mock_glob
    ):
    ...
    def test_add_buildfile_entry(self, mock_save, mock_get_entry_dir_id, mock_makedirs):
        # Test adding a buildfile entry
        tm = TopologyManager(topology_dir="test_dir", topology_object="test.pkl")
        tm.add_buildfile_entry("mock_build.yaml")
        mock_get_entry_dir_id.assert_called_once_with("mock_build.yaml")
        mock_makedirs.assert_called_once_with(os.path.join("test_dir", "mock_dir_id"))
    ...
    @patch("terphenyl_simulations.build.TopologyManager.save")
    @patch("os.makedirs")
    def test_add_structure(self, mock_makedirs, mock_save, mock_exists, mock_copy):
