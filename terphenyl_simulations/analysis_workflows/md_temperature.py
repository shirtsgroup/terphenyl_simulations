...
def parse_args():
    ...
    parser.add_argument(
        "--shaded_region",
        type=int,
        help="limits of the shaded portion of the error plots \
    ...
    parser.add_argument(
        "--gen_colvars",
        action="store_true",
        help="flag to specifify whether or not to \
    ...
    parser.add_argument(
        "--plumed",
        type=str,
        help="path to the plumed.dat file used to generate COLVARS files.",
...
def main():
    ...
    cv_names = {
        "cv1": "N Native Contacts",
        "cv2": "$R_G$ (nm)",
        "cv3": "$d_{end-to-end} (nm)$",
        "cv4": "$d_{RMSD}$",
    }
    observables = {}
    ...
    for i in range(len(dir_names)):
        ...
        for header in plumed_data.columns[2:]:
            ...
            plt.tight_layout()
            plt.savefig(
                "timeseries/" + header + "_" + str(temperatures[i]) + ".png", dpi=300
            )
            plt.close()
    ...
    for i, observable in enumerate(observables.keys()):
        ...
        if len(args.shaded_region) / (2 * (i + 1)) >= 1:
            ...
            plt.axhspan(
                ymin=args.shaded_region[0],
                ymax=args.shaded_region[1],
                color="k",
                alpha=0.1,
            )
        plt.errorbar(
            temperatures,
            observables[observable]["mean"],
            yerr=observables[observable]["std"],
        )
        ...
        plt.xticks(
            ticks=[y + 1 for y in range(len(temperatures))],
            labels=[str(int(t)) for t in temperatures],
        )
        plt.ylabel(cv_names[observable])
        plt.xlabel("Temperature (K)")
        plt.tight_layout()
