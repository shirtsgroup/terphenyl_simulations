...
def submit_all_simulations(job):
    os.chdir(job.path)
    n_jobs_old = (
        len(subprocess.check_output(["squeue", "-u", "tfobe"]).splitlines()) - 1
    )
...
def show_statepoint_table(job):
    print(
        ...
        "\n",
        "status:",
        check_production_npt_finish(job),
    )
    if os.path.exists(job.fn("WALKER0/berendsen_nvt.log")):
        tail = (
            subprocess.check_output(
                ["tail", job.fn("WALKER0/berendsen_nvt.log"), "-n", "150"]
            )
            .decode("utf-8")
            .split("\n")
        )
    ...
    if os.path.exists(job.fn("WALKER0/berendsen_npt.log")):
        tail = (
            subprocess.check_output(
                ["tail", job.fn("WALKER0/berendsen_npt.log"), "-n", "150"]
            )
            .decode("utf-8")
            .split("\n")
        )
    ...
    if os.path.exists(job.fn("WALKER0/npt_new.log")):
        tail = (
            subprocess.check_output(
                ["tail", job.fn("WALKER0/npt_new.log"), "-n", "200"]
            )
            .decode("utf-8")
            .split("\n")
        )
...
def plot_multi_CV_plot(job):
    ...
    fig, axs = plt.subplots(
        nrows=len(walker_dirs), ncols=len(walker_dirs) + 1, figsize=[20, 10]
    )
    ...
    for i, walker_dir in tqdm(enumerate(walker_dirs)):
        ...
        for j in range(len(params["CV_plots"]["x_labels"])):
            ...
            axs[j, i].plot(
                1 / 1000 * colvars["time"][::stride],
                colvars[cv_label][::stride],
                markersize=0.5,
            )
...
def sum_hills_FE(job):
    ...
    for i, (walker_dir, ax) in enumerate(zip(walker_dirs[1:], axes)):
        ...
        )  # Silences output from sum_hills
        filename = "fes.dat"
...
def plot_1D_FE_surface(job):
    ...
    for n_bins, cv_range in zip(
        params["1D_FE_plots"]["n_bins"], params["1D_FE_plots"]["cv_ranges"]
    ):
    ...
    for i, walker_dir in enumerate(walker_dirs):
        ...
        if not "WALKER0" in walker_dir:
            v_rescale = np.array(
                [v - np.max(data.values[:, -1]) for v in data.values[:, -1]]
            )
            ws = np.exp(v_rescale / kt)

        # Generate histograms weighted by unbiasing weights
        for j in range(len(cv_bins)):
            cv_hist, bin_edges = np.histogram(
                getattr(data, "cv" + str(j + 1)),
                weights=ws,
                bins=cv_bins[j],
                density=True,
            )
    ...
    fig, axes = plt.subplots(
        nrows=int(np.ceil(np.sqrt(len(cv_bins)))),
        ncols=int(np.ceil(np.sqrt(len(cv_bins)))),
        figsize=[10, 10],
    )
    axes = axes.flatten()
...
def plot_2D_FE_surface(job):
    ...
    cv1_centers = [
        (cv1_bins[i] + cv1_bins[i + 1]) / 2 for i in range(len(cv1_bins) - 1)
    ]
    cv2_bins = np.linspace(0, 4, n_bins)
    cv2_centers = [
        (cv2_bins[i] + cv2_bins[i + 1]) / 2 for i in range(len(cv2_bins) - 1)
    ]
    cv3_bins = np.linspace(0, 13, n_bins)
    cv3_centers = [
        (cv3_bins[i] + cv3_bins[i + 1]) / 2 for i in range(len(cv3_bins) - 1)
    ]
    ...
    for i, walker_dir in enumerate(walker_dirs):
        ...
        hist1, xedges, yedges = np.histogram2d(
            data.cv1, data.cv2, weights=ws, bins=[cv1_bins, cv2_bins], density=True
        )
        hist2, xedges, yedges = np.histogram2d(
            data.cv1, data.cv3, weights=ws, bins=[cv1_bins, cv3_bins], density=True
        )
        hist3, xedges, yedges = np.histogram2d(
            data.cv2, data.cv3, weights=ws, bins=[cv2_bins, cv3_bins], density=True
        )
        ...
        if not "WALKER0" in walker_dir:
            ...
            hist1, xedges, yedges = np.histogram2d(
                data.cv1, data.cv2, bins=[cv1_bins, cv2_bins], density=True
            )
            hist2, xedges, yedges = np.histogram2d(
                data.cv1, data.cv3, bins=[cv1_bins, cv3_bins], density=True
            )
            hist3, xedges, yedges = np.histogram2d(
                data.cv2, data.cv3, bins=[cv2_bins, cv3_bins], density=True
            )
