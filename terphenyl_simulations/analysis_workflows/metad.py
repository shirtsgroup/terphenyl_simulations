...
def submit_all_simulations(job):
    os.chdir(job.path)
    n_jobs_old = (
        len(subprocess.check_output(["squeue", "-u", "thfo9888"]).splitlines()) - 1
    )
...
def plot_CV_bias(job):
    ...
    plt.figure(dpi=150)
    fig, ax = plt.subplots(
        len(walker_dirs) + 2, 1, figsize=[20, 3 * len(walker_dirs) + 5]
    )
    ax[0].set_title(
        "SIGMA: "
        + str(job.sp.sigma)
    ...
    for i, walker_dir in zip(walker_ids, walker_dirs):
        ...
        ax[i].plot(
            1 / 1000 * walker_data["time"][::stride],
            walker_data.values[:, -2][::stride],
        )
        ax[i].set_ylabel("Replica " + str(i) + " $N_H$")
        ax[len(walker_dirs)].plot(
            1 / 1000 * walker_data["time"][::stride],
            walker_data.values[:, -1][::stride],
        )
    hills_file = job.fn("HILLS")
    hills_data = read_plumed_hills_file(hills_file)
    ax[len(walker_dirs)].set_ylabel("Bias Energy (kJ/mol)")
    ax[len(walker_dirs) + 1].plot(
        1 / 1000 * hills_data["time"], hills_data["height"], "o", markersize=0.5
    )
    ...
    plt.savefig(
        job.fn("CV_sampling.png"), dpi=300, transparent=False, bbox_inches="tight"
    )
    plt.close()
...
def calculate_sum_hills_FE(job):
    ...
    os.chdir(job.fn(""))
    subprocess.run(
        ["plumed", "sum_hills", "--hills", "HILLS", "--kt", str(kt), "--mintozero"]
    ...
    plt.xlim([0, 7])
    plt.plot(
        filtered.values[:, 0], filtered.values[:, 1] - np.min(filtered.values[:, 1])
    )
    plt.title(
        "SIGMA: "
        + str(job.sp.sigma)
...
def write_pbc_whole_rep(job):
    ...
    if not shutil.which("mpirun"):
        print(
            "Unable to find mpirun executable! Pleaes make sure OpenMPI is "
            + "installed or accesible with `module load`"
        )
        return
    if not shutil.which("gmx_mpi"):
        print(
            "Unable to find gmx_mpi executable! Pleaes make sure Gromacs is "
            + "installed or accesible with `module load`"
        )
        return
    os.chdir(job.fn(""))
    ...
    for walker_dir in tqdm(glob.glob("WALKER*")):
        os.chdir(walker_dir)
        subprocess.Popen(
            "mpirun -np 1 gmx_mpi trjconv -f npt_new.xtc -s npt_new.tpr -pbc whole -o npt_new.whole.xtc <<<0",
            shell=True,
        ).wait()
        os.chdir(job.fn(""))
...
def write_hb_state_trajectory(job):
    ...
    for walker_dir in tqdm(glob.glob("WALKER*")):
        ...
        walker_data = pd.read_csv(
            filename, sep=" ", header=None, comment="#", names=headers
        )
    ...
    for hb_state in n_hbond_trajs.keys():
        n_hbond_trajs[hb_state].save_xtc(
            "hbond_states/" + "hb_state_" + str(hb_state) + ".xtc"
        )
