...
def construct_rmsd_matrix(traj):
    ...
    for i in range(traj.n_frames):
        rmsd = md.rmsd(traj, traj, frame = i, precentered = False, parallel = False) # RMSD between frame i and j
        rmsd_matrix[i, :] = rmsd
...
def DBSCAN_RMSD_clustering(rmsd_matrix, eps, min_samples, parallel = True):
    ...
    dbscan = DBSCAN(eps = eps, min_samples = min_samples, metric = "precomputed", n_jobs = dbscan_parallel)
...
def write_clusters_to_file(labels, traj_object, output_dir = "clustering_output", prefix = None, output_format = "gro",):
    ...
    for cluster_id in cluster_ids:
        ...
        cluster_filename = os.path.join(output_dir, prefix + "cluster_" + str(cluster_id) + "." + output_format)
...
def clustering_grid_search(file_list, top_file, selection, n_min_samples = 40, n_eps = 40, n_processes =16, eps_limits = [0.01, 0.5], min_sample_limits = [0.005, 0.25], prefix = "grid_search", frame_start = 0, frame_end = -1, frame_stride = 1):
    ...
    min_sample_values = np.linspace(min_sample_limits[0] * total_frames, min_sample_limits[1] * total_frames, n_min_samples, dtype = int)
    ...
    for i, eps in enumerate(tqdm(eps_values)):
        for j, ms in enumerate(min_samples_values):
            dbscan, labels = DBSCAN_RMSD_clustering(rmsd_matrix, eps, ms, parallel = n_processes)
    ...
    # SS figure
    plot_grid_search(ss, min_samples_values, [round(eps,2) for eps in eps_values], "Min. Samples", '$\epsilon_{DBSCAN}$', "grid_search_ss.png")

    # n_clusters figure
    plot_grid_search(n_clusters, min_samples_values, [round(eps,2) for eps in eps_values], "Min. Samples", '$\epsilon_{DBSCAN}$', "grid_search_n_clusters.png")

    # combination metric figure
    plot_grid_search(combination, min_samples_values, [round(eps,2) for eps in eps_values], "Min. Samples", '$\epsilon_{DBSCAN}$', "grid_search_combo.png")
    ...
    dbscan, labels = DBSCAN_RMSD_clustering(rmsd_matrix, eps_values[max_indices[0][0]], min_samples_values[max_indices[0][1]])
    write_clusters_to_file(labels, oct_traj)

def main():
    if not os.path.exists("npt_new_preloaded.h5"):
        # Load in trajectory
        test_traj = md.load_xtc("../simulations/terphenyl_mop/tetramer_remd/t_200_350/sim0/npt_new.whole.xtc",
                                top = "../simulations/terphenyl_mop/tetramer_remd/t_200_350/sim0/npt_new.gro")

        for i in range(1, 6):
            print("Loading", "../simulations/terphenyl_mop/tetramer_remd/t_200_350/sim" + str(i) + "/npt_new.whole.xtc")
            test_traj = test_traj.join(md.load_xtc("../simulations/terphenyl_mop/tetramer_remd/t_200_350/sim" + str(i) + "/npt_new.whole.xtc",
                            top = "../simulations/terphenyl_mop/tetramer_remd/t_200_350/sim0/npt_new.gro"))
