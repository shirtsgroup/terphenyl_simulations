...
def main():
    ...
    min_sample_values = np.linspace(0.01 * total_frames, 0.125 * total_frames, n_min_samples, dtype = int)
    ...
    for i, eps in enumerate(eps_values):
        for j, ms in enumerate(min_samples_values):
            ...
            dbcvs[i,j] = DBCV(rmsd_matrix, labels)
    ...
    print(np.nanmax(m1), np.nanmin(m1))
