...
def check_walker_file(job, filename, walker_dirs = ["WALKER15"]):
    walkers = []
...
def calculate_sum_hills_FE(job):
    ...
    hs.utils.make_path("free_energy")
    os.chdir("free_energy")
    subprocess.run(["plumed", "sum_hills", "--hills", "../HILLS", "--kt", str(kt), "--stride", "160000", "--mintozero"]) # Silences output from sum_hills
...
def write_pbc_whole_rep(job):
    ...
    # Need to make sure gromacs is loaded before running this script
    # Make sure correct version of gromacs is available
    # os.system("module load gcc/10.2.0", shell=True).wait()
    # os.system("module load openmpi/3.1.6-gcc10.2.0", shell=True).wait()
    # os.system("source /jet/home/tfobe/software/gromacs-2021.5_plumed/bin/GMXRC", shell=True).wait()
...
@FlowProject.pre.isfile("WALKER15/HBOND_SUMS")
@FlowProject.pre.isfile("WALKER15/npt_new.whole.xtc")
