...
def main():
    ...
    # combinations
    statepoints = [
        {'height' : 0.5,   'sigma' : 2.5, 'bf' : 50},
        {'height' : 0.5,   'sigma' : 2.5, 'bf' : 100},
        {'height' : 0.5,   'sigma' : 2.5, 'bf' : 300},
        {'height' : 0.5,   'sigma' : 2.5, 'bf' : 500},
        {'height' : 0.001, 'sigma' : 2.5, 'bf' : 100000},
        {'height' : 0.003, 'sigma' : 2.5, 'bf' : 100000},
        {'height' : 0.005, 'sigma' : 2.5, 'bf' : 100000},
    ]
    replicas = 2
    ...
    for statepoint in statepoints:
        for r in range(replicas):
            ...
            for walker_id in range(n_walkers):
                ...
                replace_all_pattern("height", str(statepoint['height']), os.path.join(walker_dir, "plumed_hbond_mat.dat"))
                replace_all_pattern("sigma", str(statepoint['sigma']), os.path.join(walker_dir, "plumed_hbond_mat.dat"))
                replace_all_pattern("bf", str(statepoint['bf']), os.path.join(walker_dir, "plumed_hbond_mat.dat"))
                replace_all_pattern("bf", str(statepoint['bf']), os.path.join(walker_dir,  "plumed_reweight.dat"))
                replace_all_pattern("sigma", str(statepoint['bf']), os.path.join(walker_dir, "plumed_reweight.dat"))
                ...
                replace_all_pattern("temp", str(300), os.path.join(walker_dir, "plumed_hbond_mat.dat"))
