...
for dir in sim{0..63}; do cd $dir; echo $dir; for filename in npt_new*.xtc; do echo $filename; gmx trjconv -f $filename -o "${filename%.*}.whole.xtc" -s npt_new.tpr -pbc whole <<< 0; done; cd ..; done
