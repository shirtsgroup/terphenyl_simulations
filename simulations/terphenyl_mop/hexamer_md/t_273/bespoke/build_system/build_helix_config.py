...
def main():
    ...
    hexamer_u = mda.Universe("terphenyl_mop_hexamer.itp", "em_mop_hexamer.gro")
    ...
    new_hexamer_torsions = {}
    for torsion_type in hexamer_r1_torsions.keys():
        t_ids = get_torsion_ids(hexamer_u, "HEX", hexamer_r1_torsions[torsion_type])
        new_hexamer_torsions[torsion_type] = t_ids

    # Build ICE object
    ice = hs.edit_conf.InternalCoordinateEditor("em_mop_hexamer.gro", "terphenyl_mop_hexamer.itp")
    ...
    for torsion_type in list(new_hexamer_torsions.keys()):
        ...
        for t_id in new_hexamer_torsions[torsion_type]:
            print(t_id)
        ...
        if torsion_type == "p1":
           torsion_max_density +=  0 * np.pi/180
        if torsion_type == "p2":
        ...
        center_torsion_atoms = [torsion[1:3] for torsion in new_hexamer_torsions[torsion_type]]
        skip = 0
        for i in range(len(center_torsion_atoms)):
            ...
            set_non_prop_torsion(ice, prop_torsion_id, new_hexamer_torsions[torsion_type][i], torsion_max_density)
            ...
            #     else:
            #         skip += 1
    ice.write_structure("hexamer_adjusted.gro")
