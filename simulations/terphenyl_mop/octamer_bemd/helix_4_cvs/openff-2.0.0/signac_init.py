...
def main():
    ...
    cv_limits = [[0, 7], [0, 7], [0, 6], [0, 14]]
    statepoints = [
        {
            'height1' : 0,    'height2' : 2.5, 'height3' : 2.5, 'height4' : 2.5,
            'sigma1'  : 0,    'sigma2'  : 0.7, 'sigma3'  : 0.1, 'sigma4'  : 0.4,
            'bf1'     : 10e9, 'bf2'     : 500, 'bf3'     : 100, 'bf4'     : 100,
            'TEMP' : 250
         }
    ]
    replicas = 1
    ...
    for statepoint in statepoints:
        for r in range(replicas):
            ...
            for walker_id in range(n_walkers):
                ...
                replace_all_pattern("CV_ID", "cv" + str(walker_id + 1) if walker_id == 0 else str(walker_id), os.path.join(walker_dir, "plumed_multi_cv_reweight.dat"))
