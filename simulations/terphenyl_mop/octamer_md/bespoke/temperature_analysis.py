...
def main():
    ...
    cv_names = {"cv1" : "N Native Contacts", "cv2" : "$R_G$ (nm)", "cv3" : "$d_{end-to-end} (nm)$"}
    observables = {}
    ...
    for i in range(len(dir_names)):
        ...
        subprocess.run(["plumed", "--no-mpi", "driver", "--plumed", "plumed_multi_cvs.dat", "--kt", str(kt), "--mf_xtc", "npt_new.xtc", "--igro", "npt_new.gro"], stdout=subprocess.DEVNULL)
        os.chdir("..")
        ...
        for header in plumed_data.columns[2:]:
            ...
            observables[header]["mean"].append(np.mean(data_equil_ss))
            observables[header]["std"].append(np.std(data_equil_ss))
