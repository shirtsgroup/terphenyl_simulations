...
def run_clustering(rmsd_matrix, eps, ms, parallel):
    dbscan, labels = hs.clustering.DBSCAN_RMSD_clustering(
        rmsd_matrix, eps, ms, parallel=parallel, write_to_file=False
    )
...
def main():
    ...
    min_sample_values = np.linspace(
        0.01 * total_frames, 0.15 * total_frames, n_min_samples, dtype=int
    )
    ...
    with mp.Pool(processes=n_proccesses) as pool:
        outputs = pool.starmap(
            ...
            zip(
                ...
                Y.reshape(-1),
                X.reshape(-1),
                repeat(int(total_cores / n_proccesses)),
            ),
        )
