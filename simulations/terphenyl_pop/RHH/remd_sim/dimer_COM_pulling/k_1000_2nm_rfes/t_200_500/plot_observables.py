...
def main():
    ...
    for sim_dir in args.directories:
        ...
        for file_id in args.base_file_name:
            ...
            tot_energy = get_edr_obs(energy_df, "Total Energy")
            pull_energy = get_edr_obs(energy_df, "COM Pull En.")
            sim_obs[sim_dir] = {"com"    : [time, com],
                                "tot_energy" : [time, tot_energy],
                                "pull_energy" : [time, pull_energy]
                                }
    ...
    for key in sim_obs.keys():
        ax.plot(sim_obs[key]["com"][0], sim_obs[key]["com"][1])
    ...
    fig, ax = plt.subplots(figsize = [5,5], dpi = 300)
    for key in sim_obs.keys():
        ax.plot(sim_obs[key]["tot_energy"][0], sim_obs[key]["tot_energy"][1])
    ax.set_xlabel(r'Time (ns)')
    ...
    fig, ax = plt.subplots(figsize = [5,5], dpi = 300)
    for key in sim_obs.keys():
        print(len(sim_obs[key]["pull_energy"][0]), len(sim_obs[key]["pull_energy"][1]))
        ax.plot(sim_obs[key]["pull_energy"][0], sim_obs[key]["pull_energy"][1])
    ax.set_xlabel(r'Time (ns)')
    ax.set_ylabel(r'Total Energy (kJ mol$^{-1}$)')
    plt.show()
