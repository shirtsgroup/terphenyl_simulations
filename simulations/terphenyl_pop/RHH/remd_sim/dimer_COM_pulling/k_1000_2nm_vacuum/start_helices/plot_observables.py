...
def main():
    ...
    for sim_dir in args.directories:
        ...
        for file_id in args.base_file_name:
            ...
            rog_time, rog = get_ROG_distance(traj, "residue 1")
    ...
    fig, ax = plt.subplots(figsize = [5,5], dpi = 300)
    for key in sim_obs.keys():
        ax.hist(sim_obs[key]["tot_energy"][1][100:], bins = 30, alpha = 0.5, density=True)
    ax.set_xlabel(r'Total Energy (kJ mol$^{-1}$)')
    ...
    ax.set_xlabel(r'Time (ns)')
    ax.set_ylabel(r'Pull Energy (kJ mol$^{-1}$)')
    plt.tight_layout()
    ...
    fig, ax = plt.subplots(figsize = [5,5], dpi = 300)
    for key in sim_obs.keys():
        ax.plot(sim_obs[key]["rog"][0], sim_obs[key]["rog"][1], lw = 0.5, alpha = 0.5)
    ax.set_xlabel(r'Time (ns)')
    ax.set_ylabel(r'Radius of Gyration (nm)')
    plt.tight_layout()
    fig.savefig("radius_gyration.jpg")
