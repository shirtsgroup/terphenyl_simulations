...
class REMD_trajectories:
    ...
    def read_replica(self, i):
        print("Replica", i)
        with open(os.path.join(self.path, self.sim_id + str(i), self.mdp_file), "r")as f:
...
def main():
    ...
    remd_trajs = REMD_trajectories(os.path.abspath(""), "nvt_COM_pulling_gbsa", "xtc", "sim", "octamer_dimer_box.gro", np=8)
    h_bond_finder = HydrogenBondFinder(remd_trajs.trajs[0], remd_trajs.trajs[0].top)
    ...
    print(n_h_bonds_remd)
    print(n_h_bonds_remd)

    avg_h_bonds = [np.mean(h_bonds) for h_bonds in n_h_bonds_remd]
    std_h_bonds = [np.std(h_bonds) for h_bonds in n_h_bonds_remd]

    plt.figure(dpi = 300, figsize = [5, 5])
    plt.errorbar(remd_trajs.temps, avg_h_bonds, yerr=std_h_bonds)
    plt.xlabel("Temperature (K)")
    plt.ylabel("N Hydrogen Bonds")
    plt.savefig("h_bonds.jpg")
